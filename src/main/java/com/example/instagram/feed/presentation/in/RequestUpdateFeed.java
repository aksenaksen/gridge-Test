...
public record RequestUpdateFeed(
        @NotEmpty(message = EMPTY_FEED)
        @NotNull(message = EMPTY_FEED)
        @Size(max = 200, message = NOT_MATCHED_CONTENT)
        String content
) {
    public FeedUpdateCommand toCommand(Long feedId, List<String> imageUrls, Long userId) {
        return new FeedUpdateCommand(feedId, userId, content, imageUrls);
    }
}